...
for i in np.arange(c+1, len(df.FZI.to_numpy())-c+1):
        ...
        a = np.reshape(np.array([[np.reshape(df.GK.to_numpy()[i-c-1:i+c], (1,c*2+1))], [np.reshape(df.BK.to_numpy()[i-c-1:i+c], (1,c*2+1))], [np.reshape(df.RHOB.to_numpy()[i-c-1:i+c], (1,c*2+1))], [np.reshape(df.IK.to_numpy()[i-c-1:i+c], (1,c*2+1))], [np.reshape(df.DT.to_numpy()[i-c-1:i+c], (1,c*2+1))], [np.reshape(df.NGK.to_numpy()[i-c-1:i+c], (1,c*2+1))]]), (6,1+c*2))
