...
history = model.fit(x_train, y_train, batch_size = 128, epochs = 10, callbacks=callbacks, validation_split=0.2)
#модель обучения LSTM
...
df2 = pd.DataFrame(data={'FZI': c, 'FZI_pred': b})
df2.to_excel('ML_FZI.xlsx')
corrMatrix = df2.corr()
print(corrMatrix['FZI']['FZI_pred'])
